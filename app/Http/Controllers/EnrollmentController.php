...
class EnrollmentController extends Controller
{
    ...
    public function store(Request $request)
    {
       ...
       1. BASIC VALIDATION
    --------------------------------- */
        $page = strtolower((string) $request->input('page', 'form'));
        ...
        $source = $sourceMap[$page] ?? ucfirst($page);
        $isFooter = $page === 'footer';
    ...
    --------------------------------- */
        if (!$v3Passed && $request->filled('captcha_v3')) {
            try {
                ...
                $score    = $data['score'] ?? 0;
                $minScore = $env === 'production' ? 0.2 : 0.0;
...
--------------------------------- */
        try {
            ...
            Log::info('[Lead Mail] Support Email', ['email' => $supportEmail]);

            if (empty($supportEmail)) {
                Log::warning('[Lead Mail] Support email missing');
                return;
            }

            $courseNames = $this->resolveCourseNames($enrollment->courses);
            $leadSource  = data_get($enrollment, 'meta.source', 'Form');
            ...
            $messageBody =
                ...
                "Courses: {$courseNames}\n" .
                "Source: {$leadSource}\n" .
                "Submitted At: {$enrollment->created_at->format('d-m-Y H:i:s')}";

            // Send email using Blade template
            Mail::send('emails.new-lead', [
                'enrollment' => $enrollment,
                'courseNames' => $courseNames,
            ], function ($message) use ($supportEmail) {
                $message->to($supportEmail)
                    ->subject('New SkillVedika Lead');
            });
        } catch (\Throwable $e) {

            Log::error('[Lead Mail ERROR]', [
                'message' => $e->getMessage(),
                'file'    => $e->getFile(),
                'line'    => $e->getLine(),
            ]);
    ...
    private function resolveCourseNames($courses): string
    {
        if (empty($courses) || !is_array($courses)) {
            return '—';
        }

        return \App\Models\Course::whereIn('id', $courses)
            ->pluck('title')
            ->implode(', ');
    }
}
